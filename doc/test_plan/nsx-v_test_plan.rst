...
rally scenarios must be run with NSX-v environment. Configuration, enviroment
...
See the VMware NSX-v Plugin specification for more details.

Evaluation mission
==================

* Find important problems with integration of Neutron VMware NSX-v plugin.
...
VMware NSX-v plugin work  after new build delivery. Smoke tests will be used by
...
components of Fuel and MOS work effectively with Fuel VMware NSX-v plugin
without gaps in dataflow.

**Regression testing**

The goal of regression testing is to verify that key features of  Fuel VMware
NSX-v plugin  are not affected by any changes performed during preparation to
...
The goal of acceptance testing is to ensure that Fuel VMware NSX-v plugin has
