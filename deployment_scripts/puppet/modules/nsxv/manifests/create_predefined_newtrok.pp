class nsxv::create_predefined_newtrok (
  ...
  if !empty($settings['nsxv_floating_ip_range']) and !empty($settings['nsxv_floating_net_cidr']) {
    ...
    $net04_ext_cidr = $settings['nsxv_floating_net_cidr']
    $net04_ext_gw = $settings['nsxv_floating_net_gw']
    $default_net04_ext_gw = regsubst($net04_ext_cidr,'^(\d+\.\d+\.\d+)\.\d+/\d+$','\1.1')
    ...
    neutron_subnet { "${ext_net_name}_subnet" :
      ...
      tenant_name      => $os_tenant_name,
      gateway_ip       => pick($net04_ext_gw,$default_net04_ext_gw),
      enable_dhcp      => false,
  ...
  if !empty($settings['nsxv_internal_net_cidr']) {
    $net04_dns = split($settings['nsxv_internal_net_dns'], ',')
    $net04_cidr = $settings['nsxv_internal_net_cidr']
    ...
    neutron_subnet { "${net_name}_subnet" :
      ...
      tenant_name     => $os_tenant_name,
      gateway_ip      => regsubst($net04_cidr,'^(\d+\.\d+\.\d+)\.\d+/\d+$','\1.1'),
      enable_dhcp     => true,
