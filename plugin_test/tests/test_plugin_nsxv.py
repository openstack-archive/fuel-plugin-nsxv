...
class TestNSXvPlugin(TestBasic):
    ...
    def nsxv_floating_ip_to_public(self):
        ...
        aggs = hos.aggregate_list()
        for agg in aggs:
            if agg.name == 'vcenter':
                hos.aggregate_host_remove(agg.id, 'vcenter-vmcluster2')
                break
        else:
            logger.error("Aggregate 'vcenter' not found")
        logger.info('Aggs: {}'.format(aggs))

        for agg in aggs:
            if agg.name == 'vcenter02':
                hos.aggregate_host_add(agg.id, 'vcenter-vmcluster2')
                break
        else:
            logger.error("Aggregate 'vcenter02' not found")
        ...
        subnet_private_1 = hos.create_subnetwork(private_net_1, self.net1['cidr'])
        subnet_private_2 = hos.create_subnetwork(private_net_2, self.net2['cidr'])
