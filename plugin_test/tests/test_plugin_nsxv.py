...
from fuelweb_test.helpers.ssl_helpers import generate_user_own_cert
...
class TestNSXvPlugin(TestBasic):
    ...
    def check_connection_vms(self, os_conn, srv_list, remote=None,
        ...
        for srv in srv_list:
            ...
            if not destination_ip:
                destination_ip = list(srv_list_ip)
    ...
    def nsxv_metadata_mgt_disabled(self):
        """Check option nsxv_metadata_initializer in disabled state.
    ...
    def nsxv_metadata_listen_public(self):
        """Check option nsxv_metadata_listen in 'public' state.
    ...
    def nsxv_metadata_listen_management(self):
        """Check option nsxv_metadata_listen in 'management' state.
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_1],
          groups=["nsxv_config_ok_metadata"])
    @log_snapshot_after_test
    def nsxv_config_ok_metadata(self):
        ...
        Scenario:
            ...
            4. Generate certificate and upload it into field 'Certificate
               for metadata proxy'.
            5. Upload key into field 'Private key'('Private key for metadata
               certificate').
        ...
        generate_user_own_cert(pt_settings.CN, pt_settings.CERT_FILE,
                               pt_settings.KEY_FILE)
        ...
        self.enable_plugin(cluster_id,
                           {"nsxv_metadata_insecure/value": True,
