...
class TestNSXvPlugin(TestBasic):
    ...
    def nsxv_same_ip_different_tenants(self):
        ...
        Scenario:
            ...
            15. Verify that VMs with same ip on different tenants should
                ...
                to VM_3, VM_2 to VM_4 and vice versa.
    ...
    @test(depends_on=[nsxv_ha_mode],
          groups=["nsxv_associated_addresses_communication_on_port"])
    @log_snapshot_after_test
    def nsxv_associated_addresses_communication_on_port(self):
        """Verify MAC and IP addresses can communicate on the logical port.

        Scenario:
            1. Setup nsxv_ha_mode
            2. Launch 2 instances in each AZ.
            ...
            4. Configure a new IP address from the subnet not like original
               one on the instance associated with the logical port.
            5. Confirm that the instance cannot communicate with that
               IP address.
            6. Revert IP address. Configure a new MAC address on the instance
               associated with the logical port.
            7. Confirm that the instance cannot communicate with that MAC
               address and the original IP address.

        Duration 1.5 hours

        """
        cmd1 = "sudo /sbin/ifconfig eth1 down"
        cmd2 = "sudo /sbin/ifconfig eth1 up"
        cmd3 = "sudo /sbin/ifconfig eth1 192.168.0.51 netmask 255.255.255.0"
        cmd4 = "sudo /sbin/ifconfig eth1 hw ether 00:80:48:BA:d1:30"
        cmd_ip = [cmd1, cmd3, cmd2]
        cmd_mac = [cmd1, cmd4, cmd2]
        key = "address_communication"
        cluster_id = self.fuel_web.get_last_created_cluster()
        common = self.get_common(cluster_id)
        common.create_key(key)
        os_ip = self.fuel_web.get_public_vip(cluster_id)
        ...
        private_net2 = self.create_network(self.net1['name'])
        subnet_private = self.create_subnet(private_net2, self.net1['cidr'])
        self.add_subnet_to_router(router['id'], subnet_private['id'])
        ...
        self.create_instances(os_conn,
                              ...
                              security_group=sec_grp.name,
                              key_name=key)
        fip = self.create_and_assign_floating_ip(os_conn, ext_net=ext)
        time.sleep(5)
        srv_list = os_conn.get_servers()
        self.check_connection_vms(os_conn, srv_list)
        ...
        for cmd in cmd_ip:
            ...
            assert_true(
                ...
                "Command {0} finished with error: {1}".format(
                    cmd, command_result['stderr']))
        ...
        for cmd in cmd_mac:
            ...
            assert_true(
                command_result['exit_code'] == 0,
                "Command {0} finished with error! Error:{1}".format(
                    cmd, command_result['stderr']))
