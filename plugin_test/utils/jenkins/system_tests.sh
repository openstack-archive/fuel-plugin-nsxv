...
setup_bridge() {
  ...
  if sudo ip -o addr show dev $nic | grep -q $nic; then
    sudo ip addr flush dev $nic && echo "$nic addresses are cleaned"
  fi

  if sudo brctl show $bridge | grep -q $nic; then
    echo "$nic is already in the $bridge bridge"
  else
    sudo brctl addif $bridge $nic && echo "$nic is added to the $bridge"
  fi

  if sudo ip -o addr show to $ip | grep -q $bridge; then
    echo "$ip has already been assigned to the $bridge"
  else
    sudo ip addr add $ip dev $bridge && echo "$ip has been assigned to the $bridge"
    sudo ip link set dev $bridge up && echo "$bridge is up"
  fi
...
clean_old_bridges() {
  for intf in $WORKSTATION_IFS; do
    for br in $(brctl show | grep -v "bridge name" | cut -f1 -d'	'); do
      brctl show $br | grep -q $intf && sudo brctl delif $br $intf && \
        echo "$intf has been deleted from the $br"
    done
  done
}
