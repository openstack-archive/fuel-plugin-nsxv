...
for MOD in $(grep "^mod" Puppetfile | tr -d '[:punct:]' | awk '{ print $2 }'); do
  MOD_DIR="${DEPLOYMENT_DIR}/deployment_scripts/puppet/modules/${MOD}"
...
# exist
$TIMEOUT_CMD $BUNDLER_EXEC librarian-puppet install $VERBOSE --path=deployment_scripts/puppet/modules

# run librarian-puppet update to ensure the modules are checked out to the
# correct version
if [ "$UPDATE" = true ]; then
  $TIMEOUT_CMD $BUNDLER_EXEC librarian-puppet update $VERBOSE --path=deployment_scripts/puppet/modules
fi

# do a hard reset on the librarian managed modules LP#1489542
if [ "$RESET_HARD" = true ]; then
  for MOD in $(grep "^mod " Puppetfile | tr -d '[:punct:]' | awk '{ print $2 }'); do
    cd "${DEPLOYMENT_DIR}/deployment_scripts/puppet/modules/${MOD}"
    git reset --hard
  done
  cd $DEPLOYMENT_DIR
fi
